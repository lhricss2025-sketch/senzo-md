 ...
 *   ⚙️ button click → Admin panel → category screens with back buttons:
 *   [1] Media Panel      [2] Pairing
 *   [3] WhatsApp Ctrl    [4] Broadcast
 *   [5] Access & Premium [6] Channels & Gate
 *   [7] Bans             [8] Stats
 */
...
// ── Telegram force-join: Telegram channels (not WhatsApp channels) ──
// Admin adds TG channel usernames via /tgate add @channel ; users must join them to use bot
function listTgGates() {
  try {
    const p = path.join(__dirname, "..", "database", "bot.json");
    const raw = JSON.parse(fs.readFileSync(p, "utf8"));
    return (raw.tgGates || []).filter(Boolean);
  } catch { return []; }
}
function addTgGate(name) {
  const p = path.join(__dirname, "..", "database", "bot.json");
  const raw = JSON.parse(fs.readFileSync(p, "utf8"));
  raw.tgGates = raw.tgGates || [];
  const handle = name.startsWith("@") ? name : `@${name}`;
  if (!raw.tgGates.includes(handle)) raw.tgGates.push(handle);
...
function removeTgGate(name) {
  const p = path.join(__dirname, "..", "database", "bot.json");
  const raw = JSON.parse(fs.readFileSync(p, "utf8"));
  raw.tgGates = (raw.tgGates || []).filter((g) => g !== name && g !== `@${name}`);
  fs.writeFileSync(p, JSON.stringify(raw, null, 2));
}
...
function isAdmin(msg) {
  ...
  // Admin chat ID + owner number + owner telegram handle all count as admin
...
function userMenu(isAdminUser) {
  ...
  // Admin-only: button sirf admin ko nazar aayega
  if (isAdminUser) {
    rows.push([{ text: "⚙️ ADMIN PANEL", callback_data: "adm:panel" }]);
  }
  return rows;
}
...
function adminMenuBack(section) {
  return [
    [{ text: "◀ Back to Panel", callback_data: "adm:back" }],
  ];
}
...
async function sendForceJoinGate(chatId) {
  ...
  await bot.sendMessage(chatId, text, { reply_markup: { inline_keyboard: [gates.map((g) => ({ text: `➡️ Join ${g}`, url: `https://t.me/${g.replace("@", "")}` }))] }, parse_mode: "Markdown" });
}

// ── /start ──
// Sab ko user menu dikhega; admin ke liye uske andar extra "⚙️ ADMIN PANEL" button hoga.
bot.onText(/\/start/, async (msg) => {
  ...
  db.getUser(`tg_${uid}`);
  if (isAdm) {
    // Admin ko sirf user menu dikhega jisme "⚙️ ADMIN PANEL" button hoga.
    // Admin panel TABHI khulega jab admin wo button CLICK kare (adm:panel callback).
    await sendUserMenu(chatId, msg.message_id, false, true);
    return;
  }

  // Force join gate (Telegram channels set by admin via /tgate)
  const gates = listTgGates();
  if (gates.length) {
...
// ── User commands (work after passing gate, no admin check) ──
bot.onText(/^\/pair\s+(\d{8,15})$/, async (msg, match) => {
  ...
  const token = pairing.createPairRequest(number, chatId);
  // TURANT ephemeral one-shot socket se code request (connectWA ke reconnect ka intezar nahi)
  try {
...
bot.onText(/^\/code\s+([A-Z]{4}-\d{4})\s+(\d{8})$/, async (msg, match) => {
  // Admin bhi use kar sakta hai (fallback verification)
  if (!isAdmin(msg)) {
    if (listTgGates().length && !(await userPassedTgGates(msg.chat.id))) return;
  }
  const [, token, userCode] = match;
  const req = pairing.consumePairRequest(token);
  ...
  const c = db.getPairCode(req.number);
  if (c) {
    if (c === userCode) {
...
bot.onText(/^\/unpair$/, async (msg) => {
  ...
  try {
    // logout user ki request pe nahi hota — sirf status dikhate hain
...
// ── /join <referral code> — user apni referral count badhaye ──
...
bot.on("callback_query", async (q) => {
  ...
  // ── User callbacks ──
  // ── Pairing status check (from the "🔄 Check Status" button on /pair) ──
  if (section === "pairstat") {
    const number = action;
    const code = db.getPairCode(number);
    if (code) {
      await bot.answerCallbackQuery(q.id, { text: "✅ Code ready — check the message above!" });
    } else {
      await bot.answerCallbackQuery(q.id, { text: "⏳ Still generating… wait a few seconds and tap again.", show_alert: false });
    }
    return;
  }
  ...
  // ── Admin callbacks ──
  // Actor resolution: pehle q.from (click karne wala), phir q.message.chat (DM bot mein wahi admin hota hai).
  // msg.from idempotent nahi (bot user aa sakta hai) isliye usse actor ke liye use NAHI karte.
  ...
  // Defense-in-depth: admin panel access ke liye actor match OR chat ID match (DM mein chat = admin hi ho sakta hai).
  ...
  if (section === "adm") {
    switch (action) {
      ...
      case "pairguide":
        ...
        break;
      case "pairs": {
        const pend = pairing.listPendingPairs();
      ...
      case "qr": {
        // ALWAYS send a FRESH QR from memory (latestQRDataUrl), NOT the stale
        // media/qr.png file — a stale/expired QR gets "Could not link" on scan.
        let buf = WA?.getLatestQRBuffer ? WA.getLatestQRBuffer() : null;
        if (!buf) {
          // Wait up to 10s for the next `qr` event from WhatsApp (fresh QR rotates)
...
2. Ya \u0060/pair 92XXXXXXXXXX\u0060 use karein (8-digit code, scan ki zaroorat nahi)`,
      ...
      case "banrm":
        ...
        break;
      case "stats": {
        await bot.sendMessage(chatId, await statsScreenText(), { reply_markup: { inline_keyboard: adminMenuBack("stats") }, parse_mode: "Markdown" });
        break;
      }
    }
    await bot.answerCallbackQuery(q.id).catch(() => {});
  ...
  // ── Access & Premium sub-actions (buttons use "access:prem*" callback data) ──
  if (section === "access") {
    switch (action) {
      case "premadd":
        await bot.sendMessage(chatId,
...
bot.onText(/^\/broadcast\s+(.+)$/, async (msg, match) => {
  ...
  let targets = [];
  try {
    targets = Object.keys(sock.chats || {}).filter((id) => id.endsWith("@g.us") || (id.endsWith("@s.whatsapp.net") && id !== (sock.user?.id?.split(":")[0] || "") + "@s.whatsapp.net")).slice(0, 500);
  } catch {}
  if (!targets.length) return bot.sendMessage(msg.chat.id, "❌ Koi WhatsApp chats nahi mile");
  let ok = 0, fail = 0;
