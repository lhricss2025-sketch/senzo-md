...
*SENZO MD* ✓`);
  ...
  },
  {
    ...
    async execute(sock, msg, store, { args, reply }) {
      ...
      if (sub === "list") {
        const all = JSON.parse(require("fs").readFileSync(require("path").join(__dirname, "..", "database/premium.json"), "utf8"));
        let txt = "👑 *Premium List:*\n";
        for (const k of Object.keys(all)) txt += `• ${k}\n`;
        return reply(txt || "_Koi premium user nahi hai_");
      }
      if (sub === "check") {
  ...
  {
    name: "joingc", aliases: ["join"], category: "owner", desc: "Group link se join (owner)",
    ownerOnly: true,
    async execute(sock, msg, store, { args, reply }) {
  ...
  },
  {
    ...
    ownerOnly: true,
    async execute(sock, msg, store, { reply }) {
      const all = JSON.parse(require("fs").readFileSync(require("path").join(__dirname, "..", "database/banned.json"), "utf8"));
      let txt = "🚫 *Banned List:*\n";
      for (const k of Object.keys(all)) txt += `• ${k.split("@")[0]}\n`;
      await reply(txt || "_Koi banned nahi hai_");
    },
  },
  {
