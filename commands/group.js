...
const LINK_PATTERNS = [/https?:\/?\/(?:chat\.whatsapp\.com|wa\.me|whatsapp\.com)\S+/gi];
const CHANNEL_LINK_PATTERNS = [/https?:\/?\/www\.whatsapp\.com\/channel\/\S+/gi];
...
async function getSenderInfo(sock, jid) {
  try {
    const [store] = await Promise.all([null]);
    const contacts = await sock.fetchImageUrl?.(jid).catch(() => null);
    return contacts;
  } catch { return null; }
}
...
async function antiLinkWatch(sock, msg, store, ctx) {
  ...
  if (!LINK_PATTERNS.some((p) => p.test(body))) return;
...
async function channelLinkWatch(sock, msg, store, ctx) {
  ...
  const hasChannel = CHANNEL_LINK_PATTERNS.some((p) => p.test(body));
  const hasGroup = LINK_PATTERNS.some((p) => p.test(body));
